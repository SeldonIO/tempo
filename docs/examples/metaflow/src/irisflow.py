from metaflow import FlowSpec, step, Task, IncludeFile, conda, S3, Parameter
...
class IrisFlow(FlowSpec):
    ...
    run_local = Parameter('local',
                      help='Whether to run locally',
                      default=False)

    k8s_provider = Parameter('k8s_provider',
    ...
    def tempo(self):
        ...
        classifier, sklearn_model, xgboost_model = get_tempo_artifacts(local_sklearn_path, sklearn_url, local_xgb_path, xgboost_url, classifier_loc)
        ...
        classifier_files = [(os.path.join("classifier",f),os.path.join(classifier.get_tempo().details.local_folder, f))
