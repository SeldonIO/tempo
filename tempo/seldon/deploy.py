...
class SeldonDeployRuntime(Runtime):
    ...
    def __init__(self, host: str, user: str, password: str, auth_type: SeldonDeployAuthType=SeldonDeployAuthType.session_cookie, k8s_options: KubernetesOptions = None, _protocol = None):
    ...
    def deploy(self, model_details: ModelDetails):
        sd = SeldonDeployment(kind="SeldonDeployment",
                              ...
                              spec=SeldonDeploymentSpec(predictors=[
                                  PredictorSpec(
                                      graph=PredictiveUnit(implementation=Implementations[model_details.platform],
    ...
    def wait_ready(self, model_details: ModelDetails, timeout_secs=None) -> bool:
        ...
        dep_instance = SeldonDeploymentsApi(api_client)
        while not ready:
            sdep: SeldonDeployment = dep_instance.read_seldon_deployment(model_details.name, self._k8s_options.namespace)
